...
//printk("REG = 0x%04x ; VAL = 0x%04x\n", reg, val);
//if (reg == 0x18)
//{
//printk("WWWW [0x%08x] \n", &ssi->scr);
//printk("WWWW [0x%08x] = 0x%08x \n", &ssi->sacdat,  read_ssi(&ssi->scr));
// printk("WWWW [0x%08x] = 0x%08x \n", &ssi->sacdat,  read_ssi(&ssi->sacdat));
//return 0;
//}
//printk("tttt [0x%08x] \n", readl(0xc0a28010));
...
{
	int reset_gpio = fsl_ac97_data->ac97_reset;
	if (gpio_is_valid(reset_gpio)) {
		gpio_set_value(reset_gpio, 0);
		mdelay(100);
		gpio_set_value(reset_gpio, 1);
		mdelay(100);
		printk("warm reset of vt1613 with gpio%d \n", reset_gpio);
		mdelay(10);
	} else {
		printk("cant use gpio number %d for AC97 codec reset.\n", reset_gpio);
	}
}
...
static void fsl_ssi_ac97_reset(struct snd_ac97 *ac97)
{
	...
	if (gpio_is_valid(reset_gpio)) {
		...
		gpio_set_value(reset_gpio, 1);
		mdelay(100);
		printk("cold reset of vt1613 with gpio%d \n", reset_gpio);
		mdelay(10);
	} else {
		printk("cant use gpio number %d for AC97 codec reset.\n", reset_gpio);
	}
...
static int fsl_ssi_probe(struct platform_device *pdev)
{
	...
	if (ac97) {
		...
		reset_gpio = of_get_named_gpio_flags(np, "ac97-reset", 0, 0);
		err = gpio_request_one(reset_gpio, GPIOF_OUT_INIT_LOW, "vt1613-reset");
		if (err)
			pr_debug("AC97 codec: failed to get reset gpio: %d\n", err);
		if (gpio_is_valid(reset_gpio)) {
			ssi_private->ac97_reset = reset_gpio;
			ssi_private->ac97_warm_reset = reset_gpio;
		}
		snd_soc_set_ac97_ops(&fsl_ssi_ac97_ops);
	} else {
	...
	if (ssi_private->imx_ac97)
		printk("AC97 AC97 AC97RRRRRRRRRRRRRRRKKKKKKKKKKKKKKKKKKKKKK\n");
	if (ssi_private->imx_ac97)
		fsl_ssi_ac97_init();
	return 0;
