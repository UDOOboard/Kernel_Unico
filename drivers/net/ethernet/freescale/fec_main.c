...
static void fec_reset_phy(struct platform_device *pdev)
{
	struct net_device *ndev = platform_get_drvdata(pdev);
	struct fec_enet_private *fep = netdev_priv(ndev);
	char *driver_can_reset;
	struct device_node *np = pdev->dev.of_node;
	
	if (!np)
                return;

        /* Do not reset if driver-can-reset is set to "no" */
        driver_can_reset = of_get_property(np, "driver-can-reset", NULL);
        if (driver_can_reset && !strcmp(driver_can_reset, "no"))
                return;
